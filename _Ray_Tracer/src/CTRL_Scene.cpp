...
Scene::Scene() : first_run(true), name(""), current_details_tab_index(0), current_thread_tab_index(0) {}
void Scene::assign_name(const string& _name) { name = _name; }
bool Scene::scene_empty() { return ((light_points.size() == 0) || (spheres.size() == 0)); }
bool Scene::no_details_added() { return ((scene_obj_specifics.size() == 0) || (scene_exe_specifics.size() == 0)); }
bool Scene::check_if_first_run() { return (first_run); }
string Scene::get_name() { return name; }
u64 Scene::how_many_details_scene() { return scene_obj_specifics.size(); }
u64 Scene::how_many_details_exe() { return scene_exe_specifics.size(); }
u16 Scene::get_details_exe_num_of_threads() { return scene_exe_specifics[current_thread_tab_index].num_of_threads; }
u16 Scene::get_details_exe_schema() { return scene_exe_specifics[current_thread_tab_index].schema; }
u16 Scene::get_details_exe_block_size() { return scene_exe_specifics[current_thread_tab_index].block_size; }
vector<Light_point>& Scene::get_lights() { return light_points; }
vector<Sphere>& Scene::get_spheres() { return spheres; }
vector<details>& Scene::get_details() { return scene_obj_specifics; }
Light_point* Scene::get_lights_ptr() { return light_points.data(); }
Sphere* Scene::get_spheres_ptr() { return spheres.data(); }
details* Scene::get_details_ptr() { return scene_obj_specifics.data(); }
...
{
    scene_obj_specifics.emplace_back(std::min(lights, static_cast<u16>(light_points.size())),
                                     std::min(sph, static_cast<u16>(spheres.size())), bounces);
}
...
void Scene::add_sphere(const d3& c_pos, const unit& rad, const unit& transparent, const unit& reflective,
