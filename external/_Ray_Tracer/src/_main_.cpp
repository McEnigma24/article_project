...
int main(int argc, char* argv[])
{
...
#ifdef WIN
    sf::RenderWindow window(sf::VideoMode(static_cast<unsigned int>(G::WIDTH), static_cast<unsigned int>(G::HEIGHT)),
                            "Ray Tracer");
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (event.mouseMove.x == G::WIDTH / 2 && event.mouseMove.y == G::HEIGHT / 2)
                    break;
