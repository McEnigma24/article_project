...
Color color_tab[num_of_colors] = {Color(197, 64, 64), Color(197, 141, 54), Color(101, 195, 54), Color(50, 193, 190),
                                  Color(72, 61, 190)};
#endif // WIN
...
// PREPARATIONS
vector<Parallel_CPU::loop_info_per_thread> Parallel_CPU::prepare_subsets_for_threads_solid_blocks(u64 num_of_threads,
                                                                                                  u64 array_size)
{
...
vector<Parallel_CPU::loop_info_per_thread> Parallel_CPU::prepare_subsets_for_threads_subsequent(u64 num_of_threads,
                                                                                                u64 array_size)
{
    vector<loop_info_per_thread> ret;
    u64 end = array_size - 1;
...
vector<Parallel_CPU::loop_info_per_thread>
Parallel_CPU::prepare_subsets_for_threads_subsequent_blocks(u64 num_of_threads, u64 block_size, u64 array_size)
{
    vector<loop_info_per_thread> ret;
    u64 end = array_size - 1;
...
#define func_call(x)                                                                                                   \
    ptr->per_pixel(x, G::Render::current_scene, ptr->get_my_pixel(), G::Render::current_scene->get_lights_ptr(),       \
                   G::Render::current_scene->get_spheres_ptr(), G::Render::current_scene->get_details_ptr());
...
Parallel_CPU::Parallel_CPU(Renderer* _ptr) : c_init(ptr) {}

// MAIN FUNCTION
void Parallel_CPU::thread_group_host_and_round_keeper(int round_counter, const int num_of_threads, u16 parallel_schema,
...
#define work(x)                                                                                                        \
    ...
    auto work_comp = [&](int thread_id, const loop_info_per_thread& info) { work(thread_job_complex_schema); };
    auto work_simple = [&](int thread_id, const loop_info_per_thread& info) { work(thread_job_simple_schema); };
    ...
    {
        if (parallel_schema == PR_solid_blocks)
            schema = prepare_subsets_for_threads_solid_blocks(num_of_threads, array_size);
        else
            schema = prepare_subsets_for_threads_subsequent(num_of_threads, array_size);

        for (int i = 0; i < num_of_threads; i++)
        {
            threads.emplace_back(work_simple, i, schema[i]);
        }
    }
    else
    {
        ...
        for (int i = 0; i < num_of_threads; i++)
        {
            threads.emplace_back(work_comp, i, schema[i]);
        }
    }
}
