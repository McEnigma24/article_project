...
Stat_Record::Stat_Record(const string& _scene_name, u16 _scene_how_many_lights, u16 _scene_how_many_spheres,
    ...
    : c_init(scene_name), c_init(scene_how_many_lights), c_init(scene_how_many_spheres), c_init(scene_how_many_bounces),
      c_init(exe_parallel_schema_index), c_init(exe_how_many_threads), exe_time_report(_exe_how_many_threads)
{
}

Stat_Record::Stat_Record() : exe_time_report(G::Render::current_scene->get_details_exe_num_of_threads())
{
...
work_group& Stat_Record::get_stats() { return exe_time_report; }
...
void Stat_Record::save(string& output)
{
    ...
    tupl variables = {rel_dev,
                      min,
                      avg,
                      max,

                      CPU_LINE("CPU") GPU_LINE("GPU"),

                      G::MODEL_NAME,

                      ARCH_X86_LINE("x86") ARCH_X64_LINE("x64") GPU_LINE("CUDA"),

                      UNIT_FLOAT_LINE("float") UNIT_DOUBLE_LINE("double"),

                      G::NUM_OF_CORES,

                      CPU_LINE(to_string(omp_get_num_procs())) GPU_LINE(to_string(0)),

                      CPU_LINE(stream.str()) GPU_LINE(to_string(0)),
                      ...
                      CPU_LINE(to_string(exe_how_many_threads)) GPU_LINE(to_string(0)),

                      CPU_LINE("measuring") GPU_LINE("BEST")};
